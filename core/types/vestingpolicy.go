...
// TODO: check order!
func (p CCDVestingPolicy) MarshalTransaction(enc *transaction.Encoder) error {
...
func (p LinearVestingPolicy) MarshalTransaction(enc *transaction.Encoder) error {
...
func (p VestingPolicy) MarshalTransaction(enc *transaction.Encoder) error {
	switch p.typ {
	case VestingPolicyTypeLinear:
		if err := enc.Encode(p.data.(*LinearVestingPolicy)); err != nil {
			return err
		}
	case VestingPolicyTypeCCD:
		if err := enc.Encode(p.data.(*CCDVestingPolicy)); err != nil {
			return err
		}
	}
